...
<title>ESP32 Guitar FX Lead</title>
...
<p>Live FX + Lead + Violin + AUX Output + Dual Loop</p>
...
<button class="purple" onclick="setMode('Violin')">Violin</button>
</div>
...
let lastCmdId = -1;
let modeList = ["Clean","Distortion","Rock","Lead","Bass","Echo","Reverb","Violin"];
let modeIndex = 0;
...
function setVolume(v){
  ...
  if(violinGain) violinGain.gain.value = parseFloat(v) * 0.25;
  lcd("Mode: " + currentMode, "V:" + currentVol + " G:" + currentGain, "1", anyLoopActive() ? "1" : "0");
}
...
function startViolinMode(){
  ...
  violinOsc = audioContext.createOscillator();
  ...
  violinFilter.frequency.value = 1800;
  violinFilter.Q.value = 1.2;

  violinVibrato = audioContext.createOscillator();
  violinVibrato.type = "sine";
  violinVibrato.frequency.value = 5.5;

  violinVibratoGain = audioContext.createGain();
  violinVibratoGain.gain.value = 4;

  violinVibrato.connect(violinVibratoGain);
  violinVibratoGain.connect(violinOsc.frequency);
  ...
  lcd("Mode: Violin", "Pitch Synth", "1", anyLoopActive() ? "1" : "0");
}

function stopViolinMode(){
  violinRunning = false;

  if(violinOsc){try{violinOsc.stop();}catch(e){} try{violinOsc.disconnect();}catch(e){} violinOsc=null;}
...
function violinPitchLoop(){
  if(!violinRunning || !analyser || !audioContext || !violinOsc || !violinGain) return;
  ...
  if(freq > 90 && freq < 1500){
    violinOsc.frequency.setTargetAtTime(freq, audioContext.currentTime, 0.03);
    violinGain.gain.setTargetAtTime(parseFloat(currentVol) * 0.30, audioContext.currentTime, 0.05);
  }else{
    violinGain.gain.setTargetAtTime(0.0, audioContext.currentTime, 0.08);
  }
...
void setup() {
  ...
  Serial.println("ESP32 Guitar FX Lead Final Started");
